defmodule Mix.Tasks.CleanupSagas do
  ...
  def run(_args) do
    ...
    # Repo を直接取得
    repo = Application.get_env(:shared, :ecto_repos, [])
    |> List.first()
    |> case do
    ...
    # アクティブな SAGA の数を確認
    count_sql = "SELECT COUNT(*) FROM sagas WHERE status IN ('started', 'processing')"
    ...
    case repo.query(count_sql, []) do
      {:ok, %{rows: [[count]]}} ->
        ...
        if count > 0 do
          ...
          case repo.query(detail_sql, []) do
            {:ok, %{rows: rows}} ->
              ...
              Enum.each(rows, fn [id, saga_type, status, created_at] ->
                IO.puts("  ID: #{id}, Type: #{saga_type}, Status: #{status}, Created: #{created_at}")
              end)
          end
          ...
          if answer == "y" do
            # 削除実行
            delete_sql = "DELETE FROM sagas WHERE status IN ('started', 'processing')"
