defmodule Shared.Infrastructure.Grpc.RetryStrategy do
    ...
    case func.() do
      ...
      {:error, %GRPC.RPCError{status: status} = error} ->
        if should_retry?(status, options.retryable_errors, attempt, options.max_attempts) do
          ...
          Logger.warning("gRPC call failed with #{status}, retrying in #{delay}ms (attempt #{attempt}/#{options.max_attempts})",
            error: inspect(error)
          )
  ...
  defp calculate_delay(attempt, %{initial_delay: initial, max_delay: max, multiplier: mult, jitter: jitter}) do
