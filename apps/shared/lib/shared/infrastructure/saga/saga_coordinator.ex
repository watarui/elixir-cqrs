defmodule Shared.Infrastructure.Saga.SagaCoordinator do
  ...
  @timeout_check_interval 30_000  # 30秒ごとにタイムアウトチェック
  @default_saga_timeout 300_000   # デフォルトは5分
  ...
  def handle_call(:list_active_sagas, _from, state) do
    sagas = Enum.map(state.active_sagas, fn {id, %{saga: saga}} ->
      Map.put(saga, :id, id)
    end)
  ...
  def handle_cast({:process_event, event}, state) do
    ...
    case find_interested_sagas(event, state) do
      ...
      interested_sagas ->
        new_state = Enum.reduce(interested_sagas, state, fn {saga_id, saga_info}, acc_state ->
  ...
  defp check_and_handle_timeouts(state) do
    Enum.reduce(state.active_sagas, state, fn {saga_id, %{module: saga_module, saga: saga}}, acc_state ->
