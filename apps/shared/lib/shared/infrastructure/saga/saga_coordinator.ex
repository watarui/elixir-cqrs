defmodule Shared.Infrastructure.Saga.SagaCoordinator do
  ...
  def init(_opts) do
    ...
    state = %{
      active_sagas: %{},  # saga_id => {saga_module, saga_state}
      saga_mappings: %{   # event_type => [saga_modules]
  ...
  def handle_cast({:handle_event, event}, state) do
    ...
    # 新しいサガを開始すべきかチェック
    new_state = if saga_modules = state.saga_mappings[event_type] do
    ...
    # 既存のサガでイベントを処理
    updated_state = Enum.reduce(new_state.active_sagas, new_state, fn {saga_id, {saga_module, saga_state}}, acc_state ->
      case saga_module.handle_event(event, saga_state) do
        {:ok, commands} ->
          ...
          # 完了または失敗したサガをチェック
          if saga_module.is_completed?(updated_saga_state) or saga_module.is_failed?(updated_saga_state) do
            ...
            # サガの状態を更新
            %{acc_state | active_sagas: Map.put(acc_state.active_sagas, saga_id, {saga_module, updated_saga_state})}
          end
  ...
  defp get_next_commands(saga_module, saga_state) do
    ...
    case saga_state.current_step do
      :reserve_inventory ->
        commands = Enum.map(saga_state.items, fn item ->
          %{
  ...
  defp dispatch_command(command) do
    ...
    # コマンドディスパッチャーを使用
    dispatcher = Application.get_env(:shared, :saga_command_dispatcher, Shared.Infrastructure.Saga.CommandDispatcher)
