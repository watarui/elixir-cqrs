defmodule CommandService.Domain.Services.BusinessRulesService do
  ...
  def can_cancel_order?(order) do
    cond do
      ...
      hours_since_creation(order) > 24 ->
        {:error, BusinessRuleError,
         %{rule: "cancellation_time_exceeded",
           context: %{order_id: order.id, hours_limit: 24}}}
    ...
    if amount >= min_amount do
      ...
      {:error, BusinessRuleError,
       %{rule: "minimum_order_amount",
         context: %{minimum: min_amount, actual: amount}}}
    end
  end
  ...
  defp validate_maximum_items(%{items: items}) do
    ...
    if item_count <= max_items do
      ...
      {:error, BusinessRuleError,
       %{rule: "maximum_items_exceeded",
         context: %{maximum: max_items, actual: item_count}}}
    end
  end
  ...
  defp validate_reservation_quantity_limit(quantity, %{max_reservation: max}) do
    if quantity <= max do
      ...
      {:error, BusinessRuleError,
       %{rule: "reservation_quantity_exceeded",
         context: %{maximum: max, requested: quantity}}}
    end
  end
  ...
  defp validate_discount_usage_limit(%{usage_limit: limit, usage_count: count}) do
    if count < limit do
      ...
      {:error, BusinessRuleError,
       %{rule: "discount_usage_limit_reached",
         context: %{limit: limit, count: count}}}
    end
  end
  ...
  defp validate_discount_conditions(%{min_amount: min}, %{total_amount: total}) do
    if total >= min do
      ...
      {:error, BusinessRuleError,
       %{rule: "discount_minimum_amount_not_met",
         context: %{minimum: min, actual: total}}}
    end
  end
  ...
  defp validate_discount_combination(%{exclusive: true}, %{discounts: discounts}) 
    ...
    {:error, BusinessRuleError,
     %{rule: "exclusive_discount_conflict",
       context: %{existing_discounts: length(discounts)}}}
  end
  
  defp validate_discount_combination(_, _), do: :ok
  
  defp requires_special_handling?(%{country: country}) do
    country in ["AQ", "SJ"]  # 南極、スバールバル諸島など
  end
