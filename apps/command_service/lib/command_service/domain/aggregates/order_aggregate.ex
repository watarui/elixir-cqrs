defmodule CommandService.Domain.Aggregates.OrderAggregate do
  ...
  @enforce_keys [:id]
  defstruct [:id, :user_id, :items, :total_amount, :status, :saga_id, :version, 
             :created_at, :updated_at, :cancelled_at, :confirmed_at, uncommitted_events: []]
  ...
  注文を作成する
  """
  @spec create(String.t(), list(map())) :: {:ok, t()} | {:error, String.t()}
  def create(user_id, items) do
    with {:ok, user_entity_id} <- EntityId.from_string(user_id),
  ...
  def process_payment(%__MODULE__{} = aggregate, payment_id) do
    event = OrderPaymentProcessed.new(%{
      order_id: aggregate.id,
  ...
  defp validate_items(items) when is_list(items) and length(items) > 0 do
    validated = Enum.reduce_while(items, {:ok, []}, fn item, {:ok, acc} ->
