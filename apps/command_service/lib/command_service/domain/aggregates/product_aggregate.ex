defmodule CommandService.Domain.Aggregates.ProductAggregate do
  ...
  @enforce_keys [:id]
  defstruct [:id, :name, :price, :category_id, :version, :deleted, :created_at, :updated_at, uncommitted_events: []]
  ...
  def update(%__MODULE__{} = aggregate, params) do
    with {:ok, updates} <- validate_updates(aggregate, params) do
      if map_size(updates) == 0 do
        ...
        event = ProductUpdated.new(Map.merge(updates, %{
          id: aggregate.id,
    ...
    case updates do
      ...
      _ ->
        ...
        case updates do
          {:error, _} = error -> error
          _ ->
            updates = if params[:category_id] do
