defmodule CommandService.Domain.ValueObjects.CategoryId do
  @moduledoc """
  カテゴリIDの値オブジェクト
  ...
  def new(_), do: {:error, "Invalid category ID type"}

  # プライベート関数 - バリデーション
  defp validate(value) when byte_size(value) > 0, do: :ok
  defp validate(_), do: {:error, "Category ID cannot be empty"}
