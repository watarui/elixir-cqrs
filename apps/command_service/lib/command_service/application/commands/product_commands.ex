  ...
  defmodule CreateProduct do
    ...
    def validate(params) do
      with {:ok, name} <- validate_name(params["name"] || params[:name]),
           ...
           {:ok, category_id} <- validate_category_id(params["category_id"] || params[:category_id]) do
        {:ok, %__MODULE__{
          name: name,
          price: price,
  ...
  defmodule UpdateProduct do
    ...
    def validate(params) do
      with {:ok, id} <- validate_id(params["id"] || params[:id]),
        ...
        {:ok, struct(__MODULE__, Map.merge(updates, %{id: id, metadata: params["metadata"] || params[:metadata]}))}
      end
    end
    ...
    defp validate_updates(params) do
      ...
      updates = if params["name"] || params[:name] do
      ...
      case updates do
        {:error, _} = error -> error
        _ ->
          updates = if params["price"] || params[:price] do
          ...
          case updates do
            {:error, _} = error -> error
            _ ->
              updates = if params["category_id"] || params[:category_id] do
