defmodule QueryService.Application.ProjectionManager do
  ...
  def handle_info(:poll_events, state) do
    ...
    case EventStore.read_all_events(state.last_position) do
      {:ok, events} when events != [] ->
        ...
        # 各イベントを処理
        new_position = Enum.reduce(events, state.last_position, fn event, pos ->
          handle_event(event, state)
          pos + 1
        end)
  ...
  defp handle_product_updated(%ProductUpdated{} = event, state) do
    ...
    case state.query_repo.get(QueryService.Infrastructure.Database.Schemas.ProductSchema, event.aggregate_id) do
      nil ->
        Logger.warning("Product not found for update: #{event.aggregate_id}")
      
      product ->
        changes = Enum.reduce(event.changes, %{}, fn
  ...
  defp handle_category_updated(%CategoryUpdated{} = event, state) do
    ...
    case state.query_repo.get(QueryService.Infrastructure.Database.Schemas.CategorySchema, event.aggregate_id) do
      nil ->
