defmodule QueryService.Application.Queries.ProductQueries do
  ...
  defmodule ListProducts do
    ...
    defstruct [:category_id, :limit, :offset, :sort_by, :sort_order, :min_price, :max_price, :metadata]
    ...
    @impl true
    def validate(params) do
      with {:ok, category_id} <- validate_category_id(params["category_id"] || params[:category_id]),
    ...
    defp validate_sort_by(field) when field in ["name", "price", "created_at", "updated_at"], do: {:ok, field}
  ...
  defmodule SearchProducts do
    ...
    @impl true
    def validate(params) do
      with {:ok, search_term} <- validate_search_term(params["search_term"] || params[:search_term]),
           {:ok, category_id} <- validate_category_id(params["category_id"] || params[:category_id]),
           {:ok, limit} <- validate_limit(params["limit"] || params[:limit]),
