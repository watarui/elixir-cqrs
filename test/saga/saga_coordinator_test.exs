defmodule Shared.Infrastructure.Saga.SagaCoordinatorTest do
  ...
  setup do
    ...
    Application.put_env(:shared, :event_store_adapter, Shared.Infrastructure.EventStore.InMemoryAdapter)

    # SagaCoordinatorを開始
    {:ok, coordinator} = SagaCoordinator.start_link(
  ...
  describe "start_saga/3" do
    test "新しいサガを開始できる", %{coordinator: coordinator} do
      ...
      assert {:ok, saga_id} = SagaCoordinator.start_saga(
        TestSaga,
        initial_data,
        %{user_id: "test_user"}
      )
    ...
    test "無効なサガモジュールではエラーを返す", %{coordinator: coordinator} do
      assert {:error, _reason} = SagaCoordinator.start_saga(
        NonExistentSaga,
        %{},
        %{}
      )
