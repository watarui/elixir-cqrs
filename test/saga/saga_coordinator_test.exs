defmodule Shared.Infrastructure.Saga.SagaCoordinatorTest do
  ...
  describe "start_saga/2" do
    test "新しいサガを開始できる" do
      ...
      active_sagas = SagaCoordinator.get_active_sagas()
  ...
  describe "handle_event/1" do
    test "OrderCreatedイベントで新しいサガを開始する" do
      ...
      # イベントを処理
      SagaCoordinator.handle_event(event)
  ...
  describe "get_active_sagas/0" do
    test "アクティブなサガのリストを取得できる" do
      # 複数のサガを開始
      {:ok, saga_id1} = SagaCoordinator.start_saga(OrderSaga, %{
        order_id: "order-1",
        user_id: "user-1",
        items: [],
        total_amount: 1000
      ...
      {:ok, saga_id2} = SagaCoordinator.start_saga(OrderSaga, %{
        order_id: "order-2",
        user_id: "user-2",
        items: [],
        total_amount: 2000
